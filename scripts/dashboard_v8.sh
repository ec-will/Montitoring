...
# EarthCast HPC System Dashboard v8
# Added detailed PBS job information
...
get_pbs_job_details() {
    ...
    while IFS= read -r line; do
        if [[ $line == *"erthch01"* ]]; then
            ...
            case $status in
                "R") status_display="${GREEN}Running${NC}" ;;
                "Q") status_display="${YELLOW}Queued${NC}" ;;
                "C") status_display="${BLUE}Completing${NC}" ;;
                "H") status_display="${ORANGE}Held${NC}" ;;
...
echo ""
echo "Auto-refresh: watch -c -n 30 $HOME/monitoring/scripts/dashboard_v8.sh"
