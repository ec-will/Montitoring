...
# EarthCast HPC System Dashboard v13
# Enhanced log analysis to distinguish between failures and waiting states
...
analyze_log_status() {
    ...
    # Check for waiting/data delay states first (before treating as failures)
    if echo "$tail_content" | grep -qi "waiting.*download\|404.*not found\|still waiting"; then
        # Check if it's actively waiting (recent activity + waiting messages)
        local age=$(get_file_age "$logfile")
        if [[ $age -lt 30 ]]; then
            # Look for what it's waiting for
            if echo "$tail_content" | grep -qi "gfs\|nomads\|ncep"; then
                echo "waiting_data:Waiting for GFS data from NOAA"
            elif echo "$tail_content" | grep -qi "download"; then
                echo "waiting_data:Waiting for data download"
            else
                echo "waiting_data:Waiting for upstream data"
            fi
        else
            # Been waiting too long, might be stuck
            echo "failed:Data wait timeout - upstream delay"
        fi
        return
    fi
    
    # Check for clear failure indicators (excluding data wait errors)
    ...
    # Check for failed errors that aren't data availability issues
    if echo "$tail_content" | grep -qi "error\|failed"; then
        # Ignore data availability errors (404, connection issues to data sources)
        if echo "$tail_content" | grep -qi "404.*not found"; then
            # This is handled above in waiting states
            local age=$(get_file_age "$logfile")
            if [[ $age -lt 30 ]]; then
                echo "waiting_data:Waiting for data availability"
            else
                echo "failed:Data unavailable - upstream delay"
            fi
        elif echo "$tail_content" | grep -qi "connection.*refused\|network.*error\|timeout"; then
    ...
    case $workflow in
        "global_wrf"|"accuwx_asia"|"accuwx_europe"|"hiresw"|"drone")
            ...
            if echo "$tail_content" | grep -q "^exit$"; then
                ...
                if echo "$tail_content" | grep -qi "complete\|finished"; then
                    echo "success"
                else
                    echo "success"
                fi
                ...
                # Check if it looks like it's still running (recent activity)
                local age=$(get_file_age "$logfile")
                if [[ $age -lt 30 ]]; then
                    # Check if it's in a waiting loop or actively processing
                    if echo "$tail_content" | grep -qi "sleep\|waiting\|processing\|downloading"; then
                        echo "running"
                    else
                        echo "running"
                    fi
...
echo "================================================================="
echo ""
echo "Legend: ●●● Green (Success/Running)  ●●○ Yellow (Warning/Waiting)  ●○○ Orange (Error/Failed)  ○○○ Red (Critical)  ◐◐◐ Blue (Scheduled)"
echo ""
