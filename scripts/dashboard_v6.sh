...
# EarthCast HPC System Dashboard v6
# Added "Next Run" column for core and real-time workflows
...
get_next_schedule() {
    ...
    case $workflow in
        ...
        "hiresw")
            ...
            ;;
        "mrms")
            # Every 5 minutes at :01, :06, :11, :16, :21, :26, :31, :36, :41, :46, :51, :56
            local current_min=$(date -u +%M)
            ...
            # If we're past :56, next is :01 of next hour
            local next_hour=$(((CURRENT_HOUR + 1) % 24))
            local diff=$((61 - current_min))
            echo "$(printf "%02d:01 (%dm)" $next_hour $diff)"
            ;;
        "drone")
            local next_min=$(minutes_until $CURRENT_HOUR 10)
            if [[ $next_min -gt 50 ]]; then
                # Next hour
                local next_hour=$(((CURRENT_HOUR + 1) % 24))
                echo "$(printf "%02d:10 (%dm)" $next_hour $next_min)"
            else
                echo "$(printf "%02d:10 (%dm)" $CURRENT_HOUR $next_min)"
            fi
            ;;
    esac
}
...
get_upcoming_jobs() {
    ...
    for minute in 03 06 10 13 32 30 40; do
        for hour in $(seq 0 23); do
            ...
            if [[ $diff -gt 0 && $diff -le 720 ]]; then  # next 12 hours (720 minutes)
                case $minute in
    ...
    for job in "${major_jobs[@]}"; do
        ...
        local diff=$((job_total - current_total))
        if [[ $diff -gt 0 && $diff -le 720 ]]; then  # next 12 hours
            upcoming_jobs+=("$job")
        fi
    done
    
    # Sort by time and return first 8 (more for 12-hour window)
...
echo ""
echo "Auto-refresh: watch -c -n 30 $HOME/monitoring/scripts/dashboard_v6.sh"
