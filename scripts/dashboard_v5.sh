...
# EarthCast HPC System Dashboard v5
# Fixed column alignment and 12-hour forward look
...
printf "  %s %-20s %s\n" "$wrf_symbol" "Global WRF" "$wrf_detail"
...
printf "  %s %-20s %s\n" "$asia_symbol" "AccuWeather Asia" "$asia_detail"
...
printf "  %s %-20s %s\n" "$euro_symbol" "AccuWeather Europe" "$euro_detail"
...
printf "  %s %-20s %s\n" "$hiresw_symbol" "HiResW Processing" "$hiresw_detail"
...
printf "  %s %-20s %s\n" "$mrms_symbol" "MRMS Radar" "$mrms_detail"
...
printf "  %s %-20s %s\n" "$drone_symbol" "Drone Weather" "$drone_detail"
...
echo ""
echo "Auto-refresh: watch -c -n 30 $HOME/monitoring/scripts/dashboard_v5.sh"
