...
# EarthCast HPC System Dashboard v12
# Added log content analysis for success/failure detection
...
# Function to analyze log file for success/failure
analyze_log_status() {
    local logfile="$1"
    local workflow="$2"
    ...
    local tail_content=$(tail -10 "$logfile" 2>/dev/null)
    
    # Check for clear failure indicators first
    if echo "$tail_content" | grep -qi "killed\|error\|failed\|abort\|fatal\|exception"; then
        # Get the specific error for context
        local error_msg=$(echo "$tail_content" | grep -i "killed\|error\|failed\|abort\|fatal\|exception" | tail -1 | cut -c1-50)
    ...
    case $workflow in
        "global_wrf"|"accuwx_asia"|"accuwx_europe"|"hiresw"|"drone")
            ...
            elif echo "$tail_content" | grep -qi "complete\|finished"; then
                echo "success"
            else
            ...
            ;;
        "mrms")
            # MRMS logs end with timestamp and exit when successful
...
get_workflow_status() {
    ...
    case $status_type in
        "success")
            ...
            ;;
        "failed")
            echo "error:[Failed] $(echo $error_detail | cut -c1-30)"
            ;;
        "running")
            echo "good:[Running] Started: $(format_time $age_minutes) ago"
            ;;
        "no_log")
...
echo "================================================================="
echo ""
echo "Legend: ●●● Green (Success/Normal)  ●●○ Yellow (Warning)  ●○○ Orange (Error/Failed)  ○○○ Red (Critical)  ◐◐◐ Blue (Scheduled)"
echo ""
