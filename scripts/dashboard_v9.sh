...
# EarthCast HPC System Dashboard v9
# Fixed color formatting issues in PBS job details
...
# Function to get PBS job details
get_pbs_job_details() {
    local job_info=$(qstat -u erthch01 2>/dev/null | grep erthch01)
    if [[ -z "$job_info" ]]; then
    ...
    while IFS= read -r line; do
        if [[ $line == *"erthch01"* ]]; then
            local job_id=$(echo "$line" | awk '{print $1}' | cut -d'.' -f1)
            local jobname=$(echo "$line" | awk '{print $4}')
            local status=$(echo "$line" | awk '{print $10}')
            local nodes=$(echo "$line" | awk '{print $6}')
            local tasks=$(echo "$line" | awk '{print $7}')
            ...
            if [[ ${#jobname} -gt 20 ]]; then
                jobname="${jobname:0:17}..."
            fi
            
            # Format the line with colors properly applied
            case $status in
                "R") printf "    %-8s %-20s ${GREEN}%-10s${NC} (%s nodes, %s tasks)\n" "$job_id" "$jobname" "$status_display" "$nodes" "$tasks" ;;
                "Q") printf "    %-8s %-20s ${YELLOW}%-10s${NC} (%s nodes, %s tasks)\n" "$job_id" "$jobname" "$status_display" "$nodes" "$tasks" ;;
                "C") printf "    %-8s %-20s ${BLUE}%-10s${NC} (%s nodes, %s tasks)\n" "$job_id" "$jobname" "$status_display" "$nodes" "$tasks" ;;
                "H") printf "    %-8s %-20s ${ORANGE}%-10s${NC} (%s nodes, %s tasks)\n" "$job_id" "$jobname" "$status_display" "$nodes" "$tasks" ;;
                *) printf "    %-8s %-20s %-10s (%s nodes, %s tasks)\n" "$job_id" "$jobname" "$status_display" "$nodes" "$tasks" ;;
            esac
        fi
    done <<< "$job_info"
}
...
# PBS Queue Status with job details
...
echo ""
echo "Auto-refresh: watch -c -n 30 $HOME/monitoring/scripts/dashboard_v9.sh"
