#!/bin/bash
# Web Server Script to Generate Cluster Usage Scatter Plot
...
HTML_FILE="$SCRIPT_DIR/cluster_usage_scatter.html"
PYTHON_SCRIPT="$SCRIPT_DIR/web_server_scatter_plot.py"
...
echo "Generating cluster usage scatter plot..."
cd "$SCRIPT_DIR"
...
if python3 "$PYTHON_SCRIPT" "$JSON_FILE" "$HTML_FILE"; then
    ...
    echo "Error: Failed to generate scatter plot"
    exit 1
fi
