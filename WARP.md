...
# Test JSON export
./scripts/dashboard_json.sh --json
...
1. Add log analysis patterns in `analyze_log_status()` in `dashboard_v18.sh`
2. Define expected schedules in `get_next_schedule()`
3. If WRF-based, add output file counting logic in `get_wrfout_count()`
4. Update workflow list in both `dashboard_v18.sh` and `dashboard_json.sh`
5. Add workflow monitoring in `update_dashboard.py` if needed for web dashboard
